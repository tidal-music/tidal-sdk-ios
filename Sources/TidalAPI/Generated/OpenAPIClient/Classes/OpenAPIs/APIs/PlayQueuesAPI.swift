...
internal class PlayQueuesAPI {
     ...
     */
    @available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    internal class func playQueuesIdRelationshipsChangeEventStreamGet(id: String, include: [String]? = nil) async throws -> PlayQueuesChangeEventStreamSingleRelationshipDataDocument {
        do {
            return try await playQueuesIdRelationshipsChangeEventStreamGetWithRequestBuilder(id: id, include: include).execute().body
        } catch let httpError as HTTPErrorResponse {
            throw ErrorResponse.fromHTTPError(httpError)
        }
        // URLError and other errors propagate as-is
    ...
    internal class func playQueuesIdRelationshipsChangeEventStreamGetWithRequestBuilder(id: String, include: [String]? = nil) -> RequestBuilder<PlayQueuesChangeEventStreamSingleRelationshipDataDocument> {
        var localVariablePath = "/playQueues/{id}/relationships/changeEventStream"
        let idPreEscape = "\(APIHelper.mapValueToPathItem(id))"
        let idPostEscape = idPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{id}", with: idPostEscape, options: .literal, range: nil)
        let localVariableURLString = OpenAPIClientAPI.basePath + localVariablePath
        ...
        let localVariableRequestBuilder: RequestBuilder<PlayQueuesChangeEventStreamSingleRelationshipDataDocument>.Type = OpenAPIClientAPI.requestBuilderFactory.getBuilder()
