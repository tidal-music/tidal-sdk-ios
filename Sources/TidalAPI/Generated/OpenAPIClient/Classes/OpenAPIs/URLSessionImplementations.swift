...
extension ErrorResponse {
    static func fromHTTPError(_ httpError: HTTPErrorResponse) -> ErrorResponse {
        .error(
...
private let defaultURLSession: URLSession = {
	let cacheMemoryCapacity = 50 * 1024 * 1024
	let cacheDiskCapacity = 100 * 1024 * 1024
	...
	configuration.urlCache = URLCache(memoryCapacity: cacheMemoryCapacity, diskCapacity: cacheDiskCapacity)
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
    ...
    /**
     Override async execute() to properly separate transport errors (URLError)
     from HTTP errors (HTTPErrorResponse) with full response metadata.
     */
    @available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    override open func execute() async throws -> Response<T> {
        do {
            return try await super.execute()
        } catch let errorResponse as ErrorResponse {
            if case let .error(statusCode, data, urlResponse, underlyingError) = errorResponse {
                // Transport errors (status -1, -2): throw URLError directly
                if statusCode == -1 || statusCode == -2 {
                    throw underlyingError
                }

                // HTTP errors (4xx, 5xx): convert to HTTPErrorResponse with full response
                let httpError = HTTPErrorResponse(
                    statusCode: statusCode,
                    response: urlResponse ?? URLResponse(),  // Pass full response with headers
                    data: data  // Already optional, pass as-is
                )
                throw httpError
            }
            // Fallback: rethrow as-is
            throw errorResponse
        }
    }
