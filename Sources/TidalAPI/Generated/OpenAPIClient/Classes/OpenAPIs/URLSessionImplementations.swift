...
public protocol URLSessionProtocol {
	/// Task which performs the network fetch. Expected to be from URLSession.dataTask(with:completionHandler:) such that a network
	/// request
...
extension URLSession: URLSessionProtocol {
	/// Passthrough to URLSession.dataTask(with:completionHandler) since URLSessionDataTask conforms to URLSessionDataTaskProtocol
	/// and fetches the network data.
...
// MARK: - URLSessionRequestBuilderFactory
...
public typealias OpenAPIClientAPIChallengeHandler = (URLSession, URLSessionTask, URLAuthenticationChallenge) -> (
...
// MARK: - URLSessionDecodableRequestBuilder
...
private extension Data {
	...
	//
	// Rather than littering my code with calls to `dataUsingEncoding` to convert strings to Data, and then add that data to the
	// Data, this wraps it in a nice convenient little extension to Data. This converts using UTF-8.
	//
...
// MARK: - JSONDataEncoding + ParameterEncoding
