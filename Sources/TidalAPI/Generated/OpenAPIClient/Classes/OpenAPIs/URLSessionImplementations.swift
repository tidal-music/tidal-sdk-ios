...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
    ...
    /**
     Override async execute() to unwrap ErrorResponse and throw underlying errors directly.
     This allows URLErrors and other URLSession errors to be thrown without wrapping.
     */
    ...
    override open func execute() async throws -> Response<T> {
        do {
            ...
            // Unwrap and throw the underlying error directly if it exists
            if case let .error(statusCode, _, _, underlyingError) = errorResponse {
                // Status codes -1 and -2 are used for non-HTTP errors (URLSession errors)
                // Throw the underlying error directly instead of wrapped ErrorResponse
                if statusCode == -1 || statusCode == -2 {
                    throw underlyingError
                }
            }
            // For actual HTTP errors (4xx, 5xx), keep ErrorResponse
            throw errorResponse
        }
    }
