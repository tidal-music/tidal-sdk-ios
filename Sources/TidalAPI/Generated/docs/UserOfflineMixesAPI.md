...
```swift
    open class func userOfflineMixesIdGet(id: String, locale: String? = nil, include: [String]? = nil, completion: @escaping (_ data: UserOfflineMixesSingleResourceDataDocument?, _ error: Error?) -> Void)
```
...
// Get single userOfflineMixe.
UserOfflineMixesAPI.userOfflineMixesIdGet(id: id, locale: locale, include: include) { (response, error) in
