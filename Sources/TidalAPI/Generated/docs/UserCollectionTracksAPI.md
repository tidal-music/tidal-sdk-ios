...
```swift
    open class func userCollectionTracksIdGet(id: String, locale: String? = nil, include: [String]? = nil, replaceMedia: String? = nil, completion: @escaping (_ data: UserCollectionTracksSingleResourceDataDocument?, _ error: Error?) -> Void)
```
...
// Get single userCollectionTrack.
UserCollectionTracksAPI.userCollectionTracksIdGet(id: id, locale: locale, include: include, replaceMedia: replaceMedia) { (response, error) in
...
// Get items relationship (\"to-many\").
UserCollectionTracksAPI.userCollectionTracksIdRelationshipsItemsGet(id: id, pageCursor: pageCursor, sort: sort, locale: locale, include: include, replaceMedia: replaceMedia) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
